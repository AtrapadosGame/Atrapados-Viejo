...
public class RageSpline : MonoBehaviour, IRageSpline
{
	...
	public bool LockPhysicsColliderCount;
	...
	public void GenerateMesh(bool refreshTriangulation)
	{
		...
		if(renderer != null)
			if (renderer.sharedMaterials.GetLength(0) > 1) multipleMaterials = true;
			...
			switch (GetPhysics()) {
				...
				case Physics.Boxed:
					...
					if (lastPhysicsVertsCount != GetSplits(physicsColliderCount, 0f, 1f).Length || boxColliders[0] == null) {
						...
						var splits = GetSplits(physicsColliderCount, 0f, 1f);
	...
	{
		if (style == null || styleLocalPhysicsColliderCount) {
			if (LockPhysicsColliderCount) return vertexCount;
...
public class Mathfx {
	...
	public static Vector3 Add(Vector3 v1, Vector3 v2, Vector3 v3, Vector3 v4, Vector3 v5) {
	...
	public static Vector3 Mult(Vector3 v1, float f1, float f2) {
